...
-- Вставка сообщений в мэтчи
INSERT INTO message (match_id, sender_id, message_text, status) VALUES
        ((SELECT id FROM match WHERE user1_id = (SELECT id FROM "user" WHERE email = 'alex.ivanov@test.com')), (SELECT id FROM "user" WHERE email = 'alex.ivanov@test.com'), 'Привет! Как дела?', 1),
        ((SELECT id FROM match WHERE user1_id = (SELECT id FROM "user" WHERE email = 'alex.ivanov@test.com')), (SELECT id FROM "user" WHERE email = 'anna.smirnova@test.com'), 'Привет! Все отлично, а у тебя?', 1),
        ((SELECT id FROM match WHERE user1_id = (SELECT id FROM "user" WHERE email = 'max.petrov@test.com')), (SELECT id FROM "user" WHERE email = 'max.petrov@test.com'), 'Очень рад нашему знакомству!', 1);
