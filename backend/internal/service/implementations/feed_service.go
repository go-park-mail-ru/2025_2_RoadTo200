...
type feedService struct {
	...
	photoRepo interfaces.UserPhotoRepository // Добавляем репозиторий фото
}

func NewFeedService(userRepo interfaces.UserRepository, photoRepo interfaces.UserPhotoRepository) service.FeedService {
	return &feedService{
		userRepo:  userRepo,
		photoRepo: photoRepo, // Инициализируем репозиторий фото
	}
}
...
func (s *feedService) GetFeed(userID uuid.UUID, limit, offset int) ([]service.FeedUser, error) {
	...
	for _, user := range users {
		...
		if err != nil {
			fmt.Printf("Error converting user %s: %v\n", user.ID, err)
